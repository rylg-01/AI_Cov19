...
# === Step 1: Compute frequency of LOS values ===
...
# === Step 3: Adaptive binning with optional clinical constraints ===
bin_edges = []
bin_total = 0
...
def preprocess_image(path):
    try:
        ...
        img_tensor = img_tensor.repeat(3, 1, 1)  # Now shape is (3, 224, 224)
        return img_tensor
...
class CRDataset(Dataset):
    def __init__(self, images, labels, transform=None):
        self.images = torch.stack(images)  # Stack the image tensors together
    ...
    def __getitem__(self, idx):
        ...
        # If image is 2D (H, W), add channel dimension (1, H, W)
        if img.ndimension() == 2:
...
for fold in fold_range:
    ...
    for _, row in train_df.iterrows():
        ...
        if img is not None:
            train_imgs.append(img.squeeze(0))  # Remove the channel dimension
    ...
    for _, row in val_df.iterrows():
        img = preprocess_image(row['CR_image_paths'])
        if img is not None:
            val_imgs.append(img.squeeze(0))  # Remove the channel dimension
    ...
    scheduler = None  # Will be initialized after unfreezing
    
    train_losses, val_losses = [], []
    c_index_per_epoch = []

    unfreeze_epoch = 5  # Freeze for first few epochs
